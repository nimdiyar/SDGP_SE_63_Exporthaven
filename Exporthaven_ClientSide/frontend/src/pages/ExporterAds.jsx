...
const ExporterAds = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ title: "", description: "", category: "", location: "", minPrice: "", maxPrice: "", unit: "", certifications: "" });
      setSelectedFiles([]);
      fetchAds();
    } catch (error) {
      toast.error("Failed to post ad: " + (error.response?.data?.message || error.message));
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-6">
            ...
            {user && user.role === "exporter" && (
              <button
                className="w-full md:w-auto px-6 py-3 bg-blue-800 text-white font-medium rounded-md hover:bg-teal-600 transition-colors shadow-sm"
